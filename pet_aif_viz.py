...
def get_filename(subject_map, data_config, view_id, tab, subject, condition, tracer):
...
def get_data_for_filetype(project_folder, filenames, data_type):
	file_matches = [ item for filename in filenames for item in glob(os.path.join(project_folder, filename)) ]
	if data_type.upper() == DataType.TABLE.name:
		...
		return [ os.path.relpath(f, project_folder) for f in file_matches ]


def get_subject_data(subject_map, data_config, view_config, view_id, tab, tracer):
	rows = []
	for subject, condition_map in subject_map.items():
		...
		for condition in condition_map.keys():
			filenames = get_filename(subject_map, data_config, view_id, tab, subject, condition, tracer)
			file_data = get_data_for_filetype(data_config['project_folder'], filenames, view_config[view_id]['type'])
...
def get_maxlength(data):
	return max([len(files) for tab in data for row in data[tab] for files in row['data'].values() ], default=None)
...
def create_app(data_config_file, subject_map_file, view_config_file):
	...
	for dataset in data_config['datasets']:
		...
		for id, conf in view_config.items():
			if conf['type'] == DataType.GRAPH.name:
				continue

			data_map[dataset][id] = {}
			for tab in conf['tabs']:
				data_map[dataset][id][tab['name']] = get_subject_data(subject_map, data_config, view_config, id, tab['name'], dataset)
	...
	def graph_report(tracer, view_id, subject_id=None):
		...
		for tab in view_config[view_id]['tabs']:
			...
			filename = [ item for filename in get_filename(subject_map, data_config, view_id, tab, None, None, tracer) \
				for item in glob(os.path.join(app.config['PROJECT_FOLDER'], filename)) ][0]
	...
	def img_report(tracer, view_id, subject_id=None):
		...
		# figure out shape of images being shown in each tab
		dims = {}
		for tab in view_config[view_id]['tabs']:
		...
		return render_template('img_report.html', tracer=tracer, view_id=view_id, subject_id=subject_id, data=data, max_slice=max_slice,
			max_frame=max_frame, slice=slice, frame=frame, dims=dims, max_length=get_maxlength(data))
	...
	def gen_plot():
		...
		for condition in conditions:
			filenames = [ item for filename in get_filename(subject_map, data_config, view_id, data_type, subject, condition, tracer) \
